...
# The pad tensor can only pad width and height and depth,
# for batch ones, we use concat to replace
...
def replace_dilated_convolution(op, arch, nng=None):
    ...
    if (np.array(pre_block).shape[0] == 2):
        dilation = [pre_block[0], pre_block[1]]
    else:
        dilation = [pre_block[0], pre_block[0]]

    op.attrs.update({'padding': Padding.SAME, "dilation": (1, dilation[0], dilation[1], 1)})
...
def merge_dequant_lut_quant(op, arch, nng=None):
    ...
    support_list = [Op.Exp, Op.Log, Op.Elu]
    if lut_op.type not in support_list:
        return op
...
def tflite_optimise_graph(nng, arch, force_symmetric_int_weights):
    ...
    for idx, sg in enumerate(nng.subgraphs):
        nng.subgraphs[idx] = rewrite_graph.rewrite_graph_pre_order(
            nng, sg, arch, [], [merge_dequant_lut_quant], rewrite_unsupported=True,
        )

    # Handle Pad ops
    for idx, sg in enumerate(nng.subgraphs):
        nng.subgraphs[idx] = rewrite_graph.rewrite_graph_pre_order(
            nng, sg, arch, [], [convert_pad_to_concat], rewrite_unsupported=False,
        )
    ...
    # Repalce SpaceToBatchND/BatchToSpaceND with dilated_conv
    for idx, sg in enumerate(nng.subgraphs):
        nng.subgraphs[idx] = rewrite_graph.rewrite_graph_pre_order(
            nng,
            sg,
